...
function connect() {
  ...
  ws.onmessage = (ev) => {
    ...
    else if (msg.type === 'ronPrompt') showRonPrompt(msg.tile, msg.seat);
    else if (msg.type === 'callPrompt') showCallPrompt(msg.tile, msg.candidates, msg.seat);
  ...
  for (const divKey of SEAT_DIV_ORDER) {
    ...
    if (divKey === 'self') {
      ...
      const devHand = (s.devMode && devGodView && s.devAllHands)
        ...
        : null;
      if (devHand) {
        handEl.innerHTML = devHand.hand.map((k) => tileHtml(k, false, false, true)).join('');
      } else {
...
function updateDebugPanel() {
  ...
  document.getElementById('debugJson').textContent = JSON.stringify({ inspectSeat: inspect, state }, null, 2);
}
...
function renderActionBar() {
  ...
  if (canTsumoNow) {
    ...
    tsumoBtn.onclick = () => ws.send(JSON.stringify({ type: 'tsumoWin', seat: state.yourSeat }));
...
function doDiscard() {
  ...
  ws.send(JSON.stringify({ type: 'discard', tileId: selectedTileId, riichi: riichiMode, seat: state.yourSeat }));
...
function showRonPrompt(tile, seat) { enqueuePrompt({ kind: 'ron', tile, seat }); }
function showCallPrompt(tile, candidates, seat) { enqueuePrompt({ kind: 'call', tile, candidates, seat }); }
...
function showRonPromptInner(tile, forSeat) {
  ...
  const seatLabel = (state && state.devMode) ? `(席${forSeat + 1}) ` : '';
  wrap.innerHTML = `<span style="margin-right:12px;">${seatLabel}「${tile}」でロンできます</span>`;
  const yes = document.createElement('button');
  yes.className = 'btn primary'; yes.textContent = 'ロン';
  yes.onclick = () => { ws.send(JSON.stringify({ type: 'ronDecision', accept: true, seat: forSeat })); dequeueAndShowNext(); };
  const no = document.createElement('button');
  no.className = 'btn'; no.textContent = 'スルー';
  no.onclick = () => { ws.send(JSON.stringify({ type: 'ronDecision', accept: false, seat: forSeat })); dequeueAndShowNext(); };
  wrap.appendChild(yes); wrap.appendChild(no);
  bar.innerHTML = '';
  bar.appendChild(wrap);
}

function showCallPromptInner(tile, candidates, forSeat) {
  document.getElementById('callModal').classList.remove('hidden');
  const seatLabel = (state && state.devMode) ? `(席${forSeat + 1}) ` : '';
  document.querySelector('#callModal h3').textContent = `${seatLabel}鳴きますか？`;
  document.getElementById('callCandidates').innerHTML = candidates.map((c) => `
    <button class="btn primary" onclick="respondCall(true, '${c.word}', ${forSeat})">${c.word}（${c.han}翻）で鳴く</button>
  `).join('');
  document.getElementById('callModal').dataset.forSeat = String(forSeat);
}
function respondCall(accept, word, forSeat) {
  const seat = forSeat !== undefined ? forSeat : Number(document.getElementById('callModal').dataset.forSeat);
  ws.send(JSON.stringify({ type: 'callDecision', accept, word, seat }));
